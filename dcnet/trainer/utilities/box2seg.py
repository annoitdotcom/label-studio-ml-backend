...
def box2seg(image, boxes, label):
    ...
    for box_index in range(boxes.shape[0]):
        ...
        left = [(left_top[0] + left_bottom[0]) / 2,
                (left_top[1] + left_bottom[1]) / 2]
        right = [(right_top[0] + right_bottom[0]) / 2,
        ...
        seg = cv2.fillPoly(
            seg, [poly.reshape(4, 1, 2).astype(np.int32)], int(label[box_index]))
    ...
    f = interpolate.interp1d(
        points[:, 0], points[:, 1], fill_value='extrapolate')
