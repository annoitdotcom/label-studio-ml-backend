...
class SegmentDetector(nn.Module):
    def __init__(self,
        ...
        """
        super(SegmentDetector, self).__init__()
        self.k = k
        self.serial = serial
        ...
        self.in5.apply(self._weights_init)

        self.out5 = nn.Sequential(
            nn.Conv2d(inner_channels, inner_channels //
                      4, 3, padding=1, bias=bias),
            nn.Upsample(scale_factor=8, mode="nearest"))
        self.out5.apply(self._weights_init)

        # Up-sampling layer 4
        self.up4 = nn.Upsample(scale_factor=2, mode="nearest")
        self.in4 = nn.Conv2d(in_channels[-2], inner_channels, 1, bias=bias)
        self.in4.apply(self._weights_init)
        ...
        self.out4.apply(self._weights_init)

        # Up-sampling layer 3
        self.up3 = nn.Upsample(scale_factor=2, mode="nearest")
        self.in3 = nn.Conv2d(in_channels[-3], inner_channels, 1, bias=bias)
        self.in3.apply(self._weights_init)
        ...
        self.out3.apply(self._weights_init)

        # Up-sampling layer 2
        self.in2 = nn.Conv2d(in_channels[-4], inner_channels, 1, bias=bias)
        self.in2.apply(self._weights_init)

        self.out2 = nn.Conv2d(
            inner_channels, inner_channels // 4, 3, padding=1, bias=bias)
        self.out2.apply(self._weights_init)
    ...
    def forward(self, features, gt=None, masks=None, training=False):
        ...
        # Summary all output feafutes then predict the binarize masks
...
class BinarizeDecoder(DecoderBase):
    """ A decoder for segmenting instances """

    def __init__(self, opt, **kwargs):
        super(BinarizeDecoder, self).__init__()
        self.decoder = SegmentDetector(
            in_channels=opt.network.decoder.input_channel,
