...
class MakeSegDetectionData(PreprocessBase):
    """
    ...
    def __init__(self, opt, is_training):
        super(MakeSegDetectionData, self).__init__()
        if is_training:
            self.opt = opt.dataset.train
        else:
            self.opt = opt.dataset.validation

        self.min_text_size = self.opt.make_seg_detection_data.min_text_size
        self.shrink_ratio = self.opt.make_seg_detection_data.shrink_ratio
