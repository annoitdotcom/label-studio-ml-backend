...
class RandomCropData(AugmenterBase):
    size = (640, 640)
    max_tries = 50
    ...
    def __init__(self, opt, is_training=True):
        ...
        self.size = self.opt.random_crop_data.random_crop
        self.max_tries = self.opt.random_crop_data.max_tries
        self.min_crop_side_ratio = self.opt.random_crop_data.min_crop_side_ratio

        if is_training:
            print("----\nTraining config RandomCropData")
        else:
            print("----\nValidation config RandomCropData")

        print("Size of crop", self.size)
        print("Max tries", self.max_tries)
        print("min_crop_side_ratio", self.min_crop_side_ratio)
        print("require_original_image", self.require_original_image)
    ...
    def __call__(self, data):
        ...
        all_care_polys = [line["points"]
                          for line in data["polys"] if not line["ignore"] and len(line["points"])]
