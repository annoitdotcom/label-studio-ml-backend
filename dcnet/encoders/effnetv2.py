...
class EffNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.features:
            x = layer(x)
            x_shape_size = x.shape[-1]
            if shape_size > x_shape_size:
                shape_size = x_shape_size
                feature_list.append(previous_feature)
            previous_feature = x
        feature_list.append(previous_feature)

        return feature_list[2:6]
...
class Effv2Encoder(EncoderBase):
    """ An encoder for image embedding
    """

    def __init__(self, effv2_backbone="effnetv2_s", **kwargs):
        super(Effv2Encoder, self).__init__()

        effv2_backbones_dict = {
        ...
        self.encoder = effv2_backbones_dict[effv2_backbone]()
