...
class HighResolutionModule(nn.Module):
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_layers, num_channels,
        ...
        num_newfeature = num_channels[branch_index] + \
            growth_rate*num_layers[branch_index]
        ...
        for i in range(num_branches):
            branches.append(
                self._make_one_branch(
                    i, block, num_blocks, num_layers, num_channels)
            )
...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        fuse = f1 + residual1 + residual2 + residual3 + residual4
        # f1 = self.relu(f1)
...
#     configs = anyconfig.load("/mnt/ai_filestore/home/jason/sota_exp/DCNet/config/config-adaptive-hrnet.yaml")
#     munch_configs = munch.munchify(configs)
#     hrnet = HrnetEncoder.load_opt(munch_configs)

#     with open("model_structure.txt", "w") as f:
#         summary(hrnet.cuda(), input_size=(3, 640, 640))
